...
namespace Size
{
    ...
    {
        Func<Rect, Rect, double, double, double, double, double, double, (double, double, double, double)> calculate;

        public BoundsCalculator(IList<int> args)
        {
            calculate = args.Count switch
            {
                0 => (windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY) => (windowRect.left, windowRect.top, windowRect.right - windowRect.left, windowRect.bottom - windowRect.top),
                1 => (windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY) =>
                {
                ...
                ,
                2 => (windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY) =>
                {
                    var x = (args[0] - 左透明部) * monitorDpiX / 96;
                ...
                ,
                3 => (windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY) =>
                {
                    var x = (args[0] - 左透明部) * monitorDpiX / 96;
                    var y = (args[1] - 上透明部) * monitorDpiY / 96;
                    var w = (args[2] + 左透明部 + 右透明部) * monitorDpiX / 96;
                    return (x, y, w, windowRect.bottom - windowRect.top);
                }
                ,
                _ => (windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY) =>
                {
                    // 位置とサイズを求める。引数に、モニターの DPI に応じた倍率を掛け、さらに透明部を考慮する。透明部にも DPI に応じた倍率を掛ける。
                    var x = (args[0] - 左透明部) * monitorDpiX / 96;
                    var y = (args[1] - 上透明部) * monitorDpiY / 96;
                    var w = (args[2] + 左透明部 + 右透明部) * monitorDpiX / 96;
                    var h = (args[3] + 上透明部 + 下透明部) * monitorDpiY / 96;
            ...
            double workAreaHeight)
        {
            ...
            calculate = (windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY) =>
            {
                var (x, y, w, h) = calculateMain(windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY);
                ...
                if (pY == wY && pH != wH)
                {
                    ...
                    // 0　　　　　　口口口
                    bottom = y + h - 下透明部;
                    if (bottom > wH && bottom <= pH && x + w > pX && x < pW)
                    {
                ...
                {
                    right = x + w - 右透明部;
                    if (right > wW && right <= pW && y + h > pY && y < pH)
                    {
                ...
                {
                    left = x + 左透明部;
                    if (left >= pX && left < wX && y + h > pY && y < pH)
                    {
                        x += wX - left;
                    }
                }
        ...
        public (double, double, double, double) Calculate(Rect windowRect, Rect extendedFrameBounds, double monitorDpiX, double monitorDpiY, double windowDpi)
        {
            ...
            return calculate(windowRect, extendedFrameBounds, 左透明部, 上透明部, 右透明部, 下透明部, monitorDpiX, monitorDpiY);
        }
    }
}
